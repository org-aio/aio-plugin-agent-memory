...
fun NodeDraft.validated(): NodeDraft {
    fun check(value: Boolean, message: String) {
        if (!value) throw InputFailure(message)
    }
    val name = title.trim()
    ...
    check(
        ...
        "来源地址须为 HTTP 或 HTTPS URL",
    )
    check(tags.size <= 12 && tags.all { it.trim().length in 1..32 }, "最多 12 个标签，每个不超过 32 个字符")
    check(
        aliases.size <= 16 && aliases.all { it.trim().length in 1..160 },
        "最多 16 个别名，每个不超过 160 个字符",
    )
    return copy(
        title = name,
        url = url.trim(),
        tags = tags.map(String::trim).distinct(),
        aliases = aliases.map(String::trim).distinct(),
    )
}
...
fun requireId(id: String) {
    if (id.length != 32 || id.any { it !in '0'..'9' && it !in 'a'..'f' })
        throw InputFailure("节点或关系 ID 无效")
}

/** 双向链接只解析显式的 [[标题]]，不把推测当成模型提取的事实。 */
fun wikiLinks(text: String): List<String> =
    Regex("\\[\\[([^\\[\\]\\n]{1,160})]]")
        .findAll(text)
    ...
    repeat(depth.coerceIn(0, 3)) {
        val next =
            edges
                .flatMap { edge ->
                    when {
                        edge.source in frontier -> listOf(edge.target)
                        edge.target in frontier -> listOf(edge.source)
                        else -> emptyList()
                    }
                }
                .filter { it !in visited }
                .distinct()
                .take((limit - visited.size).coerceAtLeast(0))
                .toSet()
        visited.addAll(next)
        frontier = next
    }
