...
internal fun MemoryDialogs(state: MemoryState) {
    when (val dialog = state.dialog) {
        is MemoryDialog.Source -> ReceiptDialog(state, dialog.value)
        is MemoryDialog.History -> RevisionDialog(state, dialog)
        is MemoryDialog.Review -> ReviewDialog(state, dialog)
        is MemoryDialog.Node -> NodeEditor(state, dialog.existing)
        is MemoryDialog.Edge -> EdgeEditor(state, dialog.source)
        is MemoryDialog.Delete ->
            AlertDialog(
                ...
                confirmButton = {
                    TextButton(onClick = { state.remove(dialog) }, enabled = !state.busy) {
                        Text("确认删除", color = MaterialTheme.colorScheme.error)
                    }
                },
                dismissButton = {
                    TextButton(onClick = { state.dialog = null }, enabled = !state.busy) {
                        Text("取消")
                    }
                },
            )
...
private fun NodeEditor(state: MemoryState, existing: MemoryNode?) {
    ...
    var aliases by remember { mutableStateOf(existing?.aliases?.joinToString(", ") ?: "") }
    ...
    AlertDialog(
        ...
        text = {
            Column(
                ...
                Box {
                    OutlinedButton(onClick = { typeMenu = true }, enabled = !state.busy) {
                        Text("类型：${kind.label}")
                    }
                    DropdownMenu(typeMenu, { typeMenu = false }) {
                        NodeKind.entries.forEach { value ->
                            DropdownMenuItem(
                ...
                onClick = {
                    state.save(
                        NodeDraft(
                            ...
                            existing?.version,
                            aliases.split(',', '，').map(String::trim).filter(String::isNotEmpty),
                        ),
                        existing?.id,
                    )
                },
...
private fun EdgeEditor(state: MemoryState, source: String) {
    ...
    AlertDialog(
            ...
            Column(
                ...
                Box {
                    OutlinedButton(onClick = { expanded = true }, enabled = !state.busy) {
                        Text(target?.title ?: "选择目标节点")
                    }
                    DropdownMenu(expanded, { expanded = false }, Modifier.heightIn(max = 240.dp)) {
                        state.graph.nodes
                            .filter { it.id != source && it.title.contains(search, true) }
                            .forEach { node ->
                                DropdownMenuItem(
        ...
        confirmButton = {
            TextButton(
                onClick = {
                    target?.let { state.link(EdgeDraft(source, it.id, relation, evidence)) }
                },
                enabled = target != null && relation.isNotBlank() && !state.busy,
            ) {
                Text("保存关系")
            }
        },
