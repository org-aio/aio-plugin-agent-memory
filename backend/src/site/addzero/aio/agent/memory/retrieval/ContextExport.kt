...
internal fun MemoryStore.exportContext(request: ContextRequest): ContextResult {
    if (
        request.nodeIds.isEmpty() ||
            request.nodeIds.size > 24 ||
            request.depth !in 0..3 ||
            request.maxCharacters !in 1000..48000
    )
    ...
    for (edge in relations) {
        ...
        if (text.length + line.length + 60 > request.maxCharacters) {
            truncated = true
            break
        }
        text.append(line)
