...
进入“工作空间 → 智能体”即可对话收件和查看本轮激活图谱；独立记忆工作台位于“工作空间 → 记忆”，默认进入“随心记”，支持录入、Markdown 预览、全文详情、编辑、删除、搜索筛选和分页。保存与修改经过秘密隔离和加密，失败保留输入，编辑携带版本防止覆盖。模型未配置时继续保存加密资料并支持本地检索，wiki 整理等待空间绑定可用模型。生产发布与数据库副本验收记录见 [AIO 宿主部署文档](https://github.com/zjarlin/aio-idea/blob/main/deploy/252/README.md)。

Open “工作空间 → 智能体” (Workspace → Agent) to converse and view this turn's activated graph; the standalone workbench is at “工作空间 → 记忆” (Workspace → Memory). “随心记” (Quick Capture) supports capture, Markdown previews, full details, editing, deletion, search, filters and pagination. Saves isolate secrets and encrypt originals; failures preserve input, and edits carry a version to prevent overwrites. When no model is configured, encrypted material still saves and local retrieval works; wiki organization waits for a usable model bound to the space. Production release and database-replica acceptance records are in the [AIO 宿主部署文档](https://github.com/zjarlin/aio-idea/blob/main/deploy/252/README.md) (AIO host deployment doc).
